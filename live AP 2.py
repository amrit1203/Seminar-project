...
import datetime 
import csv
from get_stations import get_us_stations, get_nz_stations
...
def livedata1():
    ...
    date = (datetime.date.today() - datetime.timedelta(days=8)).strftime("%Y-%m-%d")
    stations = get_nz_stations()
    ...
    for s in range(0, len(stations), 50):
        ...
        with closing(requests.get(url = url, params=params, stream=True)) as req:
            ...
            for row in url_content:
                ...
                cursor.execute("INSERT INTO DataSource2.dbo.Weather VALUES(?, ?, ?, ?, ?, ?)",
                   row["STATION"], 
                   row["DATE"], 
